...
async function getAllConfigVless(hostName) {
  try {
    ...
    for (const entry of listProxy) {
      ...
      const vlessTls = `vless://${generateUUIDv4()}@{hostName}:443?encryption=none&security=tls&sni=gendar.ari-andika.site&fp=randomized&type=ws&host=gendar.ari-andika.site&path=${pathFixed}#${hostName} (${data.countryCode})`;
