...
async function getAllConfigVless(env, hostName, listProxy) {
  const listProxyElements = listProxy
    .map(({ proxyIP, proxyPort, country, isp }, index) => {
      const vlessTls = `vless://${generateUUIDv4()}@quiz.vidio.com:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2Fvl%3D${proxyIP}%3D${proxyPort}#${hostName} ${isp}`;
      ...
      const trojanTls = `trojan://${generatePASSWD()}@quiz.vidio.com:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2Ftr%3D${proxyIP}%3D${proxyPort}#${hostName} ${isp}`;
    ...
    .config-block h4 { font-size: 20px; }
      ...
      <body>
        <div class="popup">
          ...
          <a href="https://t.me/Noir7R" class="watermark" target="_blank"></a>
        </div>
        <script>
          function filterTabs() {
