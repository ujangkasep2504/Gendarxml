...
async function getAllConfigVless(env, hostName, listProxy) {
  const listProxyElements = listProxy
    .map(({ proxyIP, proxyPort, country, isp }, index) => {
      ...
      const vlessNtls = `vless://${generateUUIDv4()}@quiz.vidio.com:80?path=%2Fvl%3D${proxyIP}%3D${proxyPort}&security=none&encryption=none&host=${hostName}&fp=randomized&type=ws&sni=${hostName}#(${country}) ${isp}`;
